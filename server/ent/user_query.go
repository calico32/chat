...
type UserQuery struct {
	...
	withPrivateMessages         *PrivateMessageQuery
	withPrivateMessagesReceived *PrivateMessageQuery
...
// QueryPrivateMessages chains the current query on the "private_messages" edge.
func (uq *UserQuery) QueryPrivateMessages() *PrivateMessageQuery {
	query := &PrivateMessageQuery{config: uq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := uq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
// QueryPrivateMessagesReceived chains the current query on the "private_messages_received" edge.
func (uq *UserQuery) QueryPrivateMessagesReceived() *PrivateMessageQuery {
	query := &PrivateMessageQuery{config: uq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := uq.prepareQuery(ctx); err != nil {
			return nil, err
...
func (uq *UserQuery) Clone() *UserQuery {
	...
	return &UserQuery{
		...
		withPrivateMessages:         uq.withPrivateMessages.Clone(),
		withPrivateMessagesReceived: uq.withPrivateMessagesReceived.Clone(),
...
// WithPrivateMessages tells the query-builder to eager-load the nodes that are connected to
// the "private_messages" edge. The optional arguments are used to configure the query builder of the edge.
func (uq *UserQuery) WithPrivateMessages(opts ...func(*PrivateMessageQuery)) *UserQuery {
	query := &PrivateMessageQuery{config: uq.config}
	for _, opt := range opts {
		opt(query)
	}
	uq.withPrivateMessages = query
	return uq
}

// WithPrivateMessagesReceived tells the query-builder to eager-load the nodes that are connected to
// the "private_messages_received" edge. The optional arguments are used to configure the query builder of the edge.
func (uq *UserQuery) WithPrivateMessagesReceived(opts ...func(*PrivateMessageQuery)) *UserQuery {
	query := &PrivateMessageQuery{config: uq.config}
	for _, opt := range opts {
		opt(query)
	}
	uq.withPrivateMessagesReceived = query
	return uq
}
...
func (uq *UserQuery) sqlAll(ctx context.Context) ([]*User, error) {
	var (
		...
		loadedTypes = [3]bool{
			uq.withMessages != nil,
			uq.withPrivateMessages != nil,
			uq.withPrivateMessagesReceived != nil,
		}
	)
	...
	if query := uq.withPrivateMessages; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		nodeids := make(map[string]*User)
		for i := range nodes {
			fks = append(fks, nodes[i].ID)
			nodeids[nodes[i].ID] = nodes[i]
			nodes[i].Edges.PrivateMessages = []*PrivateMessage{}
		}
		query.withFKs = true
		query.Where(predicate.PrivateMessage(func(s *sql.Selector) {
			s.Where(sql.InValues(user.PrivateMessagesColumn, fks...))
		}))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
		...
		for _, n := range neighbors {
			fk := n.user_private_messages
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "user_private_messages" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "user_private_messages" returned %v for node %v`, *fk, n.ID)
			}
			node.Edges.PrivateMessages = append(node.Edges.PrivateMessages, n)
		}
	}

	if query := uq.withPrivateMessagesReceived; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		nodeids := make(map[string]*User)
		for i := range nodes {
			fks = append(fks, nodes[i].ID)
			nodeids[nodes[i].ID] = nodes[i]
			nodes[i].Edges.PrivateMessagesReceived = []*PrivateMessage{}
		}
		query.withFKs = true
		query.Where(predicate.PrivateMessage(func(s *sql.Selector) {
			s.Where(sql.InValues(user.PrivateMessagesReceivedColumn, fks...))
		}))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
		...
		for _, n := range neighbors {
			fk := n.user_private_messages_received
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "user_private_messages_received" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "user_private_messages_received" returned %v for node %v`, *fk, n.ID)
			}
			node.Edges.PrivateMessagesReceived = append(node.Edges.PrivateMessagesReceived, n)
		}
	}
