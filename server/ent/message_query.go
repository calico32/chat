...
func (mq *MessageQuery) sqlAll(ctx context.Context) ([]*Message, error) {
	...
	if mq.withAuthor != nil || mq.withParent != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := mq.withParent; query != nil {
		...
		nodeids := make(map[string][]*Message)
		for i := range nodes {
			if nodes[i].message_children == nil {
				continue
			}
			fk := *nodes[i].message_children
			if _, ok := nodeids[fk]; !ok {
				ids = append(ids, fk)
			}
			nodeids[fk] = append(nodeids[fk], nodes[i])
		...
		for _, n := range neighbors {
			nodes, ok := nodeids[n.ID]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "message_children" returned %v`, n.ID)
			}
			for i := range nodes {
	...
	if query := mq.withChildren; query != nil {
		...
		for i := range nodes {
			...
			nodes[i].Edges.Children = []*Message{}
		}
		query.withFKs = true
		...
		for _, n := range neighbors {
			fk := n.message_children
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "message_children" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "message_children" returned %v for node %v`, *fk, n.ID)
			}
